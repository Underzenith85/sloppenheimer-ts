  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const scheduleRetry = (
        ...
        Effect.gen(function* () {
          const existing = state.retries.get(issueId)
          ...
          const fiber = yield* Effect.forkScoped(
            Effect.sleep(delay).pipe(
              Effect.zipRight(Queue.offer(mailbox, { _tag: 'RetryDue', issueId, attempt })),
              Effect.asVoid,
            ),
          )
          state.retries.set(issueId, { issueId, identifier, attempt, dueAt, error, fiber })
        ...
        switch (event._tag) {
          ...
          case 'WorkerExited': {
            ...
            if (event.outcome === 'normal') {
              ...
              yield* scheduleRetry(event.issueId, entry.issue.identifier, 1, null, true)
            } else {
              yield* scheduleRetry(
                event.issueId,
                entry.issue.identifier,
