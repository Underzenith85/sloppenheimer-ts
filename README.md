...
The client follows the protocol generated by the installed `codex app-server` and uses newline-
delimited JSON on stdout with a 10 MB per-line limit. Stderr is diagnostic output only and is never
parsed as protocol data. Command, file-change, legacy, and permission approvals are accepted for
the session. MCP elicitations are declined, and a blocking `item/tool/requestUserInput` request
fails the worker attempt because Symphony has no interactive operator channel. Unadvertised dynamic
tools receive a schema-valid failed tool result; other unsupported server requests receive a
method-not-found response, so neither path stalls the turn.
