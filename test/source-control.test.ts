...
describe('host Git source control', (): void => {
  it('prepares protected main and publishes a deterministic agent diff', async (): Promise<void> => {
    ...
    expect(await readFile(join(fixture.workspace, 'README.md'), 'utf8')).toBe('base\n')
    ...
    expect(await git(fixture.remote, ['rev-parse', 'refs/heads/symphony/issue-165'])).toBe(
      published._tag === 'Published' ? published.headSha : '',
    )
  ...
  it('reports an empty diff without creating a remote branch', async (): Promise<void> => {
    const fixture = await makeGitRepository()
    roots.push(fixture.root)
    const sourceControl = makeGitSourceControl({
      remoteUrl: fixture.remote,
    ...
    await expect(Effect.runPromise(sourceControl.publish(issue, prepared))).resolves.toEqual({
      _tag: 'NoChanges',
      branchName: 'symphony/issue-165',
